...
from tensorflow.keras.layers import Dense, GlobalAveragePooling2D, Dropout,concatenate, Activation
...
def Network_config(class_num=4, epoch=50, initial_epoch=0, batch_size=32,
                     train_data=None, train_label=None,
                     test_data=None, test_label=None, fold=0):
    adam = Adam(lr=0.01, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0009)
    ...
    out_ = Dense(class_num, activation="softmax")(x)
    model = Model(inputs=input_tensor, outputs=out_)
    ...
    callbacks = [tensorboard, CSV_record]
    gc.disable()
    model.fit_generator(
...
def main():
    # Network_config()
    data = creat_list('../CreateRandomList/four_tea_category.txt')
