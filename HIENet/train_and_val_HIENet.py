...
from tensorflow.keras.layers import Conv2D, MaxPooling2D, Input, Flatten, BatchNormalization
...
def cross_validation(data, test, K, epoch, class_num, batch_size):
    category = len(data)
    print(category)
    print("=========================")
    # if shuffle:
    ...
    for i in range(0,K):
        ...
        test_label = []
        for c in range(category):
            # part_train_data_path, part_test_data_path = tools.slice_train_test(data[c], i, K)
            part_train_data_path = data[c]
            part_test_data_path = test[c]
            for train_len in range(len(part_train_data_path)):
                train_data_path.append(part_train_data_path[train_len])
                train_label.append(c)
            for test_len in range(len(part_test_data_path)):
                test_data_path.append(part_test_data_path[test_len])
                test_label.append(c)

        print("train data:", len(train_data_path), len(train_label))
        print("test data:", len(test_data_path), len(test_label))
...
def main():
    # Network_config()
    test_iphone7 = creat_list('../CreateRandomList/test_hwmate10_tea_grades_list.txt')
    data = creat_list('../CreateRandomList/three_tea_grades_list.txt')
    cross_validation(data, test_iphone7, K=10, epoch=50, class_num=5, batch_size=32)
    # cross_validation(data, K=10, epoch=50, class_num=14, batch_size=32)
